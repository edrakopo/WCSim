...
G4float WCSimWLSObject::GetCollectionEfficiency(float angle)
{
    return Interpolate_func(angle, 10, GetCollectionEfficiencyAngle(), GetCollectionEfficiencyArray())/100.;
}

G4float WCSimWLSObject::Interpolate_func(G4float x, G4int ncount, G4float *angle, G4float *quantity){
...
G4float* WCSimWLSObject::GetCollectionEfficiencyAngle(){
...
// This can be overridden by setting GetCE in the derived class
G4float* WCSimWLSObject::GetCollectionEfficiencyArray(){
...
G4String basicWLS::GetWLSName() {G4String WLSName = "wls"; return WLSName;}
//G4double basicWLS::GetExposeHeight() {return 29.*mm;} 
//G4double basicWLS::GetRadius() {return 39.*mm;}
//G4double basicWLS::GetPMTGlassThickness() {return 0.40*cm;}
G4float basicWLS::HitTimeSmearing(float Q) {
...
G4float* basicWLS::Getqpe() //currently uses the same as 20inch
{
...
G4float* basicWLS::GetQE(){
  static G4float QE[20] = { 0.00, .005, .09, .21, .28, .30, .29, .28, .26, .24, .22, .18, .13, .075, .04, .02, .008, 0.00, 0.00, 0.00};
  return QE;
}
G4float* basicWLS::GetQEWavelength(){static G4float wavelength[20] = { 260., 280., 300., 320., 340., 360., 380., 400., 420., 440., 460., 480., 500., 520., 540., 560., 580., 600., 620., 640.};
  return wavelength;}

G4float  basicWLS::GetmaxQE(){
  const G4float maxQE = 0.30;
  return maxQE;
}

// Should be actual PMT Dark Rate, not effective dark rate in detector including other LE noise
G4float basicWLS::GetDarkRate(){
...
G4float basicWLS::GetDarkRateConversionFactor(){
  const G4float factor = 1.367;
  return factor;
}

G4int basicWLS::GetNbOfQEDefined(){
...
// COMBINED WLSS CLASS
WCSimBasicWLSObject::WCSimBasicWLSObject(){
}

WCSimBasicWLSObject::WCSimBasicWLSObject(std::map<G4float,G4float> mQE){
  mapQE=mQE;
...
WCSimBasicWLSObject::WCSimBasicWLSObject(std::vector<G4float> wl,std::vector<G4float> qe,G4float max){
  wavelength=wl;
  QE=qe;
  maxQE=max;
}

void WCSimBasicWLSObject::DefineQEHist(std::map<G4float,G4float> mapQE){
  gQE = new TGraph();
  G4int iPt=0;
  std::map<G4float, G4float>::iterator itr;
