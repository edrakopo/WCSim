...
//////////////// Construct only WLS Plate //////////////
WCSimDetectorConstruction::WLSMap_t WCSimDetectorConstruction::WLSLogicalVolumes;

G4LogicalVolume* WCSimDetectorConstruction::ConstructWLSPlate(G4String PMTName, G4String CollectionName, G4String detectorElement){

  WLSKey_t key(PMTName,CollectionName);

  WLSMap_t::iterator it = WLSLogicalVolumes.find(key);
  if (it != WLSLogicalVolumes.end()) {
      //G4cout << "Restore WLS" << G4endl;
       return it->second;
  }
...
/*
G4LogicalVolume* WCSimDetectorConstruction::ConstructPMTAndWLSPlate(G4String PMTName, G4String CollectionName, G4String detectorElement){
  //G4cout<<"in ConstructPMTAndWLSPlate--------"<<G4endl;
  PMTKey_t key2(PMTName,CollectionName);

  PMTMap_t::iterator it = PMTLogicalVolumes.find(key2);
  if (it != PMTLogicalVolumes.end()) {
     //G4cout << "Restore PMT" << G4endl;
      return it->second;
  }
 ...
 // Instantiate a new sensitive detector 
  ...
  if( ! SDman->FindSensitiveDetector(SDName, false) ) {
   ...
   logicWCODWLSPlate->SetSensitiveDetector( aWCPMT );

   PMTLogicalVolumes[key2] = logicWCPMT;
